...
#            ->(value) { value.present? ? value.reject(&:blank?) : [] })
  ...
  def tag_list
    self.tags.respond_to?(:each) ? self.tags.join(', ') : ''
  end
  ...
  def search
    [query_string, author_id_filter, year_filter, tags_filter, sorting]
      .compact.reduce(:merge)
  end

  def sorting
    index.order(SORT[(sort || 'relevance').to_sym])
  end

  def query_string
    index
      .query(query_string: { fields: [:title, :author, :description],
